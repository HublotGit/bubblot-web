(function () {
    ...
    function mapMapCtrl($scope) {
        ...
        function findData(element) {
            ...
            $scope.latitudeCursor = [], $scope.longitudeCursor = [], $scope.depthCursor = [], $scope.tempCursor = [];
            ...
            for (var j = 0; j < $scope.datax.length; j++) {
                ...
                for (var i = 0; i < $scope.datax[j].length; i++) {
                    ...
                    if (infoX < $scope.datax[j][i] + 6 / $scope.zoom && infoX > $scope.datax[j][i] - 6 / $scope.zoom &&
                        infoY < $scope.datay[j][i] + 6 / $scope.zoom && infoY > $scope.datay[j][i] - 6 / $scope.zoom) {
